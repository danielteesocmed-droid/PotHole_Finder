...
async def create_report(
    ...
    db: Session = Depends(get_db),
    _rl=Depends(rate_limit(10, 60)),      # 10 submissions per IP per minute
):
    photo_path = None
...
async def get_report(request: Request, ref_code: str, db: Session = Depends(get_db), _rl=Depends(rate_limit(30, 60))):
...
async def get_stats(request: Request, db: Session = Depends(get_db), _rl=Depends(rate_limit(30, 60))):
...
async def admin_login(request: Request, body: AdminLogin, db: Session = Depends(get_db), _rl=Depends(rate_limit(10, 60))):
